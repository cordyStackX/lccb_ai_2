...
export async function POST(req: NextRequest) {
    try {
        ...
        if (password !== c_password) return NextResponse.json({ succes: false, error: "Password is not match" }, { status: 403 });

        if (password.length < 8) return NextResponse.json({ success: false, error: "Must be more than 8 characters" }, { status: 402 });
