...
export async function POST(req: NextRequest) {
    ...
    const rate = rateLimit(req, { windowMs: 1000, max: 5, keyPrefix: "fetchimg" });
    if (!rate.allowed) {
    ...
    if (data && data.length <= 0) return NextResponse.json({ success: false, error: "No Image found" }, { status: 405 });
